...
def sdf_vectorized(double t, np.ndarray x,
    ...
    cdef double step = 0.85 # the elevation of tree
    cdef double baystart = 22.51-0.46 # start location of mangrove section
...
#    cdef np.ndarray[np.float64_t, ndim=1] ty = np.array([0.46/ratio, 1.15/ratio, 1.83/ratio])
    cdef np.ndarray[np.float64_t, ndim=1] tx = np.array([0.46/ratio, 1.37/ratio, 0.46/ratio, 1.37/ratio, 0.46/ratio]) # x of trunk center (3 trees a unit)
    cdef np.ndarray[np.float64_t, ndim=1] ty = np.array([0.46/ratio, 1.15/ratio, 1.83/ratio, 2.51/ratio, 3.2/ratio]) # y of trunk center (3 trees a unit)
    cdef np.ndarray[np.float64_t, ndim=1] bayfront = np.array([0./ratio, 1.83/ratio, 3.66/ratio, 5.49/ratio, 7.32/ratio, 9.15/ratio, 10.98/ratio, 12.81/ratio, 14.64/ratio, 16.47/ratio])
    cdef np.ndarray[np.complex128_t, ndim=1] G_root = np.zeros((3,), dtype=np.complex128)
    cdef double complex num, denom
    for i in range(n):
        phi[i] = 1.0e16
        for bi in range(len(bayfront)):
            for ti in range(len(tx)):
                X_0 = x[i,0] - (tx[ti] + bayfront[bi] + baystart)
                Y_0 = x[i,1] - (ty[ti])
                Z_0 = x[i,2] - (step)
